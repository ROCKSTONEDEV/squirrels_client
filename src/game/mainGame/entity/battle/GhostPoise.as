package game.mainGame.entity.battle
{
	...
	public class GhostPoise extends BattlePoise
	{
		...
		static private const FIXTURE_DEF:b2FixtureDef = new b2FixtureDef(SHAPE, null, 0.8, 0.1, 0.3, CATEGORIES_BITS, MASK_BITS, 0);
		static private const BODY_DEF:b2BodyDef = new b2BodyDef(false, false, b2Body.b2_dynamicBody);

		static public const MAX_VELOCITY:int = 150;
		static public const RELOAD_TIME:int = 1200;

		private var targets:Array = [];

		public function GhostPoise():void
		{
			initGhoseAnim();
		}

		private function initGhoseAnim(): void {
			while(numChildren > 0)
				removeChildStarlingAt(0);

			this.view = new StarlingAdapterMovie(new GhostPoiseImage());
			this.view.loop = true;
			this.view.x = -15;
			this.view.y = -15;
		...
		override public function beginContact(contact:b2Contact):void
		{
			...
			var damage:int = 4 * (((creator as HeroBattle) == null || !(creator as HeroBattle).extraDamage) ? 1 : 2);
			if (creator != null)
