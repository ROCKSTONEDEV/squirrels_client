package game.mainGame.entity.simple
{
	...
	public class HoppingBall extends GameBody implements IMagnetizable, IPinFree, ISensor
	{
		...
		static private const JUMP_SLOWDOWN_FACTOR:int = 20;
		...
		private var hopMovie: StarlingAdapterMovie;

		public function HoppingBall():void
		{
			this.view = new StarlingAdapterMovie(new HoppingImg);
			this.view.loop = true;
			this.view.y = 10;
			this.view.stop();
			...
			hopMovie = new StarlingAdapterMovie(new HoppingOnHead());
			hopMovie.gotoAndStop(1);
			this.hopAnimation = new FlyToHeroAnimation(hopMovie);
		...
		public function beginContact(contact:b2Contact):void
		{
			...
			if (!hero || hero.isDead || hero.inHollow || hero.hasJoints("hopping") || this.hero == hero || this.prevSquirrel == hero)
				return;
		...
		public function magnetize(magnet:Magnet):void
		{
			...
			hopMovie.gotoAndStop(1);
			if (magnet && this.hopAnimation && magnet.parentStarling)
			{
		...
		private function pinSquirrel(heroId:int):void
		{
			...
			if (!hero || hero.isDead || hero.inHollow || hero.hasJoints("hopping"))
				return;
			...
			this.hopAnimation.show(FlyToHeroAnimation.TO_HERO, this.hero, 5, new Point(this.position.x * Game.PIXELS_TO_METRE, this.position.y * Game.PIXELS_TO_METRE), new Point(-5, -Hero.Y_POSITION_COEF - 10), onCompleteTo);

			this.jumpBonus = this.hero.jumpVelocity * JUMP_SLOWDOWN_FACTOR / 100;
			...
			this.jointToHero.SetUserData("hopping");
		}
		...
		{
			this.hopAnimation.x = -5;
			this.hopAnimation.y = -2 * Hero.Y_POSITION_COEF - 10;
			hopMovie.play();
