package game.mainGame.entity.joints
{
	...
	public class JointRope extends StarlingAdapterSprite implements IJoint, IComplexEditorObject, IGameObject, ISerialize, IDispose, IUpdate, IUnselectable
	{
		...
		public function JointRope():void
		{
		...
		public function update(timeStep:Number = 0):void
		{
			...
			_newPoint.setTo(start.x, start.y);
			this.rope.start = _newPoint;

			_newPoint.setTo(end.x, end.y);
			this.rope.end = _newPoint;

			pin0.x = this.rope.start.x;
			pin0.y = this.rope.start.y;
			pin1.x = this.rope.end.x;
			pin1.y = this.rope.end.y;
		}
