...
//public:
	public function b2PairManager(){
		m_pairs = new new Vector.<b2Pair>();
		m_pairBuffer = new new Vector.<b2Pair>();
		m_pairCount = 0;
		m_pairBufferCount = 0;
		m_freePair = null;
	}
	...
	*/
	...
	public function RemoveBufferedPair(proxy1:b2Proxy, proxy2:b2Proxy) : void{
		...
		if (!pair.IsBuffered())
		{
			...
			//b2Settings.b2Assert(pair.IsFinal());
	...
	public function Commit(callback:Function) : void{
		...
		for (i = 0; i < m_pairBufferCount; ++i)
		{
			...
			{
				//b2Settings.b2Assert(m_broadPhase.TestOverlap(proxy1, proxy2));
...
//public:
	...
	b2internal var m_pairs:Vector.<b2Pair>;
	private var m_freePair:b2Pair;
	b2internal var m_pairCount:int;
	
	private var m_pairBuffer:Vector.<b2Pair>;
