...
//public:
	...
	public function MoveProxy(proxy_:*, aabb:b2AABB, displacement:b2Vec2) : void {
		...
		for (axis = 0; axis < 2; ++axis)
		{
			...
			{
				index = lowerIndex;
				for (; index > 0 && lowerValue < (bounds[int(index - 1)] as b2Bound).value; )
				{
					bound = bounds[index];
			...
			{
				index = upperIndex;
				for (; index < boundCount - 1 && (bounds[int(index + 1)] as b2Bound).value <= upperValue; )
				{
					bound = bounds[ index ];
			...
			{
				index = lowerIndex;
				for (; index < boundCount - 1 && (bounds[int(index + 1)] as b2Bound).value <= lowerValue; )
				{
					bound = bounds[ index ];
					nextBound = bounds[ int(index + 1) ];
			...
			{
				index = upperIndex;
				for (; index > 0 && upperValue < (bounds[int(index - 1)] as b2Bound).value; )
				{
					bound = bounds[index];
					prevBound = bounds[int(index - 1)];
	...
	public function RayCast(callback:Function, input:b2RayCastInput):void
	{
		...
		var startValues:Vector.<uint> = new Vector.<uint>();
		var startValues2:Vector.<uint> = new Vector.<uint>();
...
//private:
	...
	b2internal var m_proxyPool:Vector.<b2Proxy> = new Vector.<b2Proxy>();
	private var m_freeProxy:b2Proxy;

	b2internal var m_bounds:Vector.<Vector.<b2Bound> > ;

	private var m_querySortKeys:Array = new Array();
	private var m_queryResults:Vector.<b2Proxy> = new Vector.<b2Proxy>();
