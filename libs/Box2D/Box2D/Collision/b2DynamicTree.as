...
package Box2D.Collision 
{
	...
	public class b2DynamicTree 
	{
		...
		*/
		...
		public function GetMRoot():b2DynamicTreeNode
		{
			return m_root;
		}
		/**
		...
		public function RayCast(callback:Function, input:b2RayCastInput):void
		{
			...
			var r:b2Vec2 = new b2Vec2(p1.x - p2.x, p1.y - p2.y);
			...
			var tY:Number;
			{
				...
				segmentAABB.lowerBound.x = p1.x < tX ? p1.x : tX;
				segmentAABB.lowerBound.y = p1.y < tY ? p1.y : tY;
				segmentAABB.upperBound.x = p1.x > tX ? p1.x : tX;
				segmentAABB.upperBound.y = p1.y > tY ? p1.y : tY;
			}
			...
			{
				node = stack[--count];
				lowerBound = node.aabb.lowerBound;
				upperBound = node.aabb.upperBound;
				...
				var separation:Number = Math.abs(v.x * (p1.x - (lowerBound.x + upperBound.x) / 2) + v.y * (p1.y - (lowerBound.y + upperBound.y) / 2))
										- (v.x > 0.0 ? v.x : -v.x) * ((upperBound.x - lowerBound.x) / 2) - ((v.y > 0.0 ? v.y : -v.y)) * ((upperBound.y - lowerBound.y) / 2);
				if (separation > 0.0)
					continue;
		...
		private function InsertLeaf(leaf:b2DynamicTreeNode):void
		{
			...
			var centerX:Number = (leaf.aabb.lowerBound.x + leaf.aabb.upperBound.x) / 2;
			var centerY:Number = (leaf.aabb.lowerBound.y + leaf.aabb.upperBound.y) / 2;
			...
			if (sibling.child1 != null)
			{
				do
				{
					...
					var norm1:Number = Math.abs((child1.aabb.lowerBound.x + child1.aabb.upperBound.x) / 2 - centerX)
									 + Math.abs((child1.aabb.lowerBound.y + child1.aabb.upperBound.y) / 2 - centerY);
					var norm2:Number = Math.abs((child2.aabb.lowerBound.x + child2.aabb.upperBound.x) / 2 - centerX)
