...
public class b2DynamicTreeBroadPhase implements IBroadPhase
{
	...
	public function UpdatePairs(callback:Function):void
	{
		...
		var fatAABB:b2AABB;
		var stack:Vector.<b2DynamicTreeNode>;
		var count:int;
		var mRoot:b2DynamicTreeNode;
		...
		for each(var queryProxy:b2DynamicTreeNode in m_moveBuffer)
		{
			...
			stack = new Vector.<b2DynamicTreeNode>();
			mRoot = m_tree.GetMRoot();
			count = 0;
			if (mRoot != null)
			{
				stack[count++] = mRoot;
				...
				for (; count > 0; )
				{
					...
					if (node.aabb.TestOverlap(fatAABB))
					{
						if (node.child1 == null)
						{
							if (node != queryProxy)
							{
								...
								var isLess:Boolean = node.key < queryProxy.key;
								pair.proxyA = isLess?node:queryProxy;
								pair.proxyB = (!isLess)?node:queryProxy;
								++m_pairCount;
							}
						}
						else
