...
public class b2ContactSolver
{
	...
	public function Initialize(step:b2TimeStep, contacts:Vector.<b2Contact>, contactCount:int, allocator:*):void
	{
		...
		for (; m_constraints.length < m_constraintCount; )
		{
		...
		for (i = 0; i < contactCount; ++i)
		{
			...
			cc.surfaceVelocityModifier = contact.surfaceVelocityModifier;
	...
	public function SolveVelocityConstraints() : void{
		...
		for (var i:int = 0; i < m_constraintCount; ++i)
		{
			...
			for (j = 0; j < c.pointCount; j++)
			{
				...
				dvX -= c.surfaceVelocityModifier * tangentX;
				dvY -= c.surfaceVelocityModifier * tangentY; 
				...
				a = ccp.tangentImpulse + lambda;
				newImpulse = a < -maxFriction ? -maxFriction : a > maxFriction ? maxFriction : a;
	...
	public function SolvePositionConstraints(baumgarte:Number):Boolean
	{
		...
		for (var i:int = 0; i < m_constraintCount; i++)
		{
			...
			for (var j:int = 0; j < c.pointCount; j++)
			{
				...
				// Prevent large corrections and allow slop.
				a = baumgarte * (separation + b2Settings.b2_linearSlop);
				var C:Number = a < -b2Settings.b2_maxLinearCorrection ? -b2Settings.b2_maxLinearCorrection : a > 0.0 ? 0.0 : a;
