	...
	*/
	...
	b2internal override function InitVelocityConstraints(step:b2TimeStep) : void{
		...
		if (m_enableLimit)
		{
			...
			var a:Number = m_upperTranslation - m_lowerTranslation;
			if ((a > 0.0 ? a : -a) < 2.0 * b2Settings.b2_linearSlop)
			{
	...
	b2internal override function SolveVelocityConstraints(step:b2TimeStep) : void{
		...
		if (m_enableMotor && m_limitState != e_equalLimits)
		{
			...
			var a:Number = m_motorImpulse + impulse;
			m_motorImpulse = a < -maxImpulse ? -maxImpulse : a > maxImpulse ? maxImpulse : a;
		...
		if (m_enableLimit && m_limitState != e_inactiveLimit)
		{
			...
			if (m_limitState == e_atLowerLimit)
			{
				m_impulse.z = m_impulse.z > 0.0 ? m_impulse.z : 0.0;
			}
			...
			var f2r:b2Vec2 = m_K.Solve22(solve22Out, bX, bY)
			f2r.x += f1.x;
			f2r.y += f1.y;
			m_impulse.x = f2r.x;
			m_impulse.y = f2r.y;
		...
		else
		{
			...
			var df2:b2Vec2 = m_K.Solve22(solve22Out, -Cdot1X, -Cdot1Y);
			m_impulse.x += df2.x;
			m_impulse.y += df2.y;
		...
		bA.m_linearVelocity.x = v1.x;
		bA.m_linearVelocity.y = v1.y;
		bA.m_angularVelocity = w1;

		bB.m_linearVelocity.x = v2.x;
		bB.m_linearVelocity.y = v2.y;
	...
	b2internal override function SolvePositionConstraints(baumgarte:Number ):Boolean
	{
		...
		var r1X:Number = m_localAnchor1.x - m_localCenterA.x;
		var r1Y:Number = m_localAnchor1.y - m_localCenterA.y;
		tX = (cos1 * r1X + (-sin1) * r1Y);
		r1Y = (sin1 * r1X + cos1 * r1Y);
		r1X = tX;
		...
		var r2X:Number = m_localAnchor2.x - m_localCenterB.x;
		var r2Y:Number = m_localAnchor2.y - m_localCenterB.y;
		tX =  (cos2 * r2X + (-sin2) * r2Y);
		r2Y = (sin2 * r2X + cos2 * r2Y);
		r2X = tX;
		...
		if (m_enableLimit)
		{
			m_axis.x = cos1 * m_localXAxis1.x + ( -sin1) * m_localXAxis1.y;
			m_axis.y = sin1 * m_localXAxis1.x + cos1 * m_localXAxis1.y;
			...
			a = m_upperTranslation - m_lowerTranslation;
			if ((a > 0.0 ? a : -a) < 2.0 * b2Settings.b2_linearSlop)
			{
				// Prevent large angular corrections.
				C2 = translation < -b2Settings.b2_maxLinearCorrection ? -b2Settings.b2_maxLinearCorrection : translation > b2Settings.b2_maxLinearCorrection ? b2Settings.b2_maxLinearCorrection : translation;
				linearError = translation > 0.0 ? translation : -translation;
				active = true;
			}
			else if (translation <= m_lowerTranslation)
			{
				// Prevent large angular corrections and allow some slop.
				a = translation - m_lowerTranslation + b2Settings.b2_linearSlop;
				C2 = a < -b2Settings.b2_maxLinearCorrection ? -b2Settings.b2_maxLinearCorrection : a > 0.0 ? 0.0 : a;
			...
			{
				// Prevent large angular corrections and allow some slop.
				a = translation - m_upperTranslation + b2Settings.b2_linearSlop;
				C2 = a < 0.0 ? 0.0 : a > b2Settings.b2_maxLinearCorrection ? b2Settings.b2_maxLinearCorrection : a;
				linearError = translation - m_upperTranslation;
				active = true;
			}
		}
		
		m_perp.x = cos1 * m_localYAxis1.x + ( -sin1) * m_localYAxis1.y;
		m_perp.y = sin1 * m_localYAxis1.x + cos1 * m_localYAxis1.y;
		...
		var C1XAbs:Number = C1X > 0.0 ? C1X : -C1X;
		linearError = linearError > C1XAbs ? linearError : C1XAbs;
		angularError = C1Y > 0.0 ? C1Y : -C1Y;
		...
		else
		{
			...
			var impulse1:b2Vec2 = m_K.Solve22(solve22Out, -C1X, -C1Y);
	...
	private var solve22Out:b2Vec2 = new b2Vec2();
};
