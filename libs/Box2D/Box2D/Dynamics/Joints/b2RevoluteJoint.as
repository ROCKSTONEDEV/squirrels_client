	...
	*/
	...
	b2internal override function InitVelocityConstraints(step:b2TimeStep) : void{
		...
		if (m_enableLimit)
		{
			...
			var a:Number = m_upperAngle - m_lowerAngle;
			if ((a > 0.0 ? a : -a) < 2.0 * b2Settings.b2_angularSlop)
			{
	...
	b2internal override function SolvePositionConstraints(baumgarte:Number):Boolean{
		...
		if (m_enableLimit && m_limitState != e_inactiveLimit)
		{
			...
			if (m_limitState == e_equalLimits)
			{
				...
				a = angle - m_lowerAngle;
				C = a < -b2Settings.b2_maxAngularCorrection ? -b2Settings.b2_maxAngularCorrection : a > b2Settings.b2_maxAngularCorrection ? b2Settings.b2_maxAngularCorrection : a;
				limitImpulse = -m_motorMass * C;
				angularError = C > 0.0 ? C : -C;
			}
			else if (m_limitState == e_atLowerLimit)
			{
				...
				a = C + b2Settings.b2_angularSlop;
				C = a < -b2Settings.b2_maxAngularCorrection ? -b2Settings.b2_maxAngularCorrection : a > 0.0 ? 0.0 : a;
				limitImpulse = -m_motorMass * C;
			}
			else if (m_limitState == e_atUpperLimit)
			{
				...
				// Prevent large angular corrections and allow some slop.
				a = C - b2Settings.b2_angularSlop;
				C = a < 0.0 ? 0.0 : a > b2Settings.b2_maxAngularCorrection ? b2Settings.b2_maxAngularCorrection : a;
				limitImpulse = -m_motorMass * C;
			}
