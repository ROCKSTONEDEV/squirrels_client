 ...
 * the child swf, it cannot automatically remove event listeners, stop NetStreams, soundObject, etc., all of which could interfere
 * with garbage collection. Therefore it is considered a best practice to [whenever possible] build each subloaded swf so that 
 * it has some sort of <code>dispose()</code> method that runs cleanup code (removes event listeners, stops soundObject, closes NetStreams, etc.).
...
function dispose(event:Event):void { 
    //do cleanup stuff here like removing event listeners, stopping soundObject, closing NetStreams, etc...
}
</listing>
 * 
			...
			if (!_scriptAccessDenied) {
				...
				if (_content != null) {
					if (this.vars.autoPlay == false && _content is MovieClip) {
						...
						st.volume = 0; //just make sure you can't hear any soundObject as it's loading in the background.
