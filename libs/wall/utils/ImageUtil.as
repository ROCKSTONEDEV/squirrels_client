package utils
{
	...
	public class ImageUtil
	{
		...
		static public function scale(bm:Bitmap, width:Number, height:Number, x:Number, y:Number):Bitmap
		{
			var cloned:Bitmap = copy(bm);

			ImageUtil.resize(cloned, width, height);
			//ImageUtil.center(cloned, width, height, x, y);
		...
		static public function load(url:String, params:Object, onComplete:Function, data:* = null):void
		{
		...
		static public function drawRectSprite(width:Number, heigth:Number, x:Number, y:Number, color:int = 0x000000, alpha:Number = 1):Sprite
		{
			var sprite:Sprite = new Sprite();
			sprite.graphics.beginFill(color, alpha);
			sprite.graphics.drawRect(0, 0, width, heigth);
